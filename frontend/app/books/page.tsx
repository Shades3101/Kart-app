...
export default function Books() {
    ...
    const [selectedCondition, setSelectedCondtion] = useState<string[]>([]);
    ...
    const toggleFilter = (section: string, item: string) => {
        ...
        switch (section) {
            case "condition":
                setSelectedCondtion(updateFilter);
                break;
            case "classType":
                                    ...
                                    <div className="mt-2 space-y-2">
                                        {values.map((value) => (
                                            <div key={value} className="flex items-center space-x-2">
                                                <Checkbox id={value} checked={key === "condition" ? selectedCondition.includes(value)
                                                    : key === "classType" ? selectedType.includes(value)
                                                        : selectedCategory.includes(value)
                                                } onCheckedChange={() => toggleFilter(key, value)} />
                                                <label htmlFor={value} className="text-sm font-medium text-gray-500 leading-none">
                                                    {value}
                                                </label>
                                            </div>
                                        ))}
                        ...
                        <>
                            <div className="flex justify-between">
                                <div className="mb-8 text-xl font-semibold">
                                    Buy Used Books Online in India
                                </div>
                                            ...
                                            <CardContent className="p-0">
                                                <Link href={`/books/${book._id}`}>
                                                    <div className="relative">
                                                        <Image src={book.images[0]} alt={book.title} width={400} height={300}
                                                            className="h-[250px] w-screen object-cover transition-transform duration-300 group-hover:scale" />
                                                        ...
                                                        <Button size="icon" variant="ghost" className="absolute right-2 top-2 h-8 w-8 rouned-full bg-white/80 backdrop-blur-sm transition-opacity duration-300 hover:bg-white group-hover:opacity-100">
